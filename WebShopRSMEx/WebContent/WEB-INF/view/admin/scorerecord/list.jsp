...
<html>
  	<head>
    	<title>评分记录列表 - 个性化购物商城推荐系统</title>
  	...
  	<body>
    	<div id="body">
	        ...
	        <form action="${pageContext.request.contextPath}/admin/scorerecord/list" method="post" id="form1">
		        <div class="barboxs">
		            <div class="leftbox">
		                  	...
		                  	<c:when test="${pageBean!=null && pageBean.list!=null && pageBean.list.size()>0}">
		                  		<c:forEach var="scorerecord" items="${pageBean.list}" varStatus="status">
				                    <tr>
				                        ...
				                        <td>${scorerecord.user.username }</td>
				                        <td>${scorerecord.item.itemname }</td>
				                        <td>${scorerecord.score }</td>
				                        <td>${scorerecord.createtime }</td>
				                        <td class="text-center">
				                            <div class="btn-group">
				                                <a href="javascript:doDelete(${scorerecord.id })" class="btn btn-xs btn-danger" title="删除">
	        ...
	        <form action="${pageContext.request.contextPath}/admin/scorerecord/list" method="post" id="form">
	...
	<script>
		...
		function doDelete(id){
		    layer.confirm("确认删除？",{icon:7,title:"删除"},function(index){
			   layer.close(index);
			   var data = "id="+id;
		       common_ajax_other("admin/scorerecord/delete",data);
		   	});
		}
	</script>
</html>